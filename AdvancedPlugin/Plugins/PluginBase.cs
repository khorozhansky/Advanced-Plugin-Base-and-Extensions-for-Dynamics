namespace AdvancedPlugin.Plugins
{
    ...
    where TTraceLog : TraceLogBase
  {
    ...
    /// <summary>Initializes a new instance of the <see cref="PluginBase{TEntity, TContext, TTraceLog}"/> class.</summary>
    ...
    protected abstract TContext BuildOrgCtx(IOrganizationService orgService);

    protected abstract TTraceLog BuildTraceLog(IPluginContextBase pluginCtx, ITracingService tracingService);

    protected abstract bool GetIsExecutionAllowed(IPluginContextBase pluginCtx);
    ...
    protected void RegisterPluginStep(Stage? stage, Mode? mode, string pluginMessage, Action<PluginContext> handler)
    {
    ...
    public void Execute(IServiceProvider serviceProvider)
    {
                                              ...
                                              this.customVerboseLogSwitchedOn))
      {
        ...
        var isExecutionAllowed = this.GetIsExecutionAllowed(pluginCtx);
        if (!isExecutionAllowed)
        {
          pluginCtx.Trace(
            $"[SERVICE OPERATION INITIATED. SKIPPING PLUGIN LOGIC.] Entity: {primaryEntityName}, Message: {messageName}, Stage: {stage}, Mode: {mode}, Depth: {pluginCtx.Depth}, UserId: {pluginCtx.UserId}, InitiatingUserId: {pluginCtx.InitiatingUserId}");
          return;
        }
        ...
        Func<PluginStep, bool> stepSearchCondition =
          step =>
            ...
            && (
              string.IsNullOrEmpty(step.PluginMessage) 
              || step.PluginMessage.Equals(messageName, StringComparison.InvariantCultureIgnoreCase))
            && (
        ...
        var execStep =
          this.RegisteredPluginSteps.Where(stepSearchCondition).FirstOrDefault();

        if (execStep == null)
        {
          pluginCtx.Trace(
            $"[NO APPROPRIATE REGISTERED STEP FOUND. SKIPPING PLUGIN LOGIC.] Entity: {primaryEntityName}, Message: {messageName}, Stage: {stage}, Mode: {mode}, Depth: {pluginCtx.Depth}, UserId: {pluginCtx.UserId}, InitiatingUserId: {pluginCtx.InitiatingUserId}");
          return;
        }

        pluginCtx.Trace("Executing the step action...");
        try
        {
          execStep.Execute(pluginCtx);
    ...
    /// </summary>
    /// <seealso cref="Microsoft.Xrm.Sdk.IPlugin" />
    public class PluginContext : IPluginContext<TContext> 
    {
      /// <summary>
      ...
      /// <summary>
      /// Backing Field for  <see cref="PreImageAsBaseEntity"/>
      /// </summary>
      private Entity preImageAsBaseEntity;

      /// <summary>
      /// Backing Field for  <see cref="PreImageEntity"/>
      /// </summary>
      private TEntity preImageEntity;

      /// <summary>
      /// Backing Field for  <see cref="PostImageAsBaseEntity"/>
      /// </summary>
      private Entity postImageAsBaseEntity;

      /// <summary>
      /// Backing Field for  <see cref="PostImageEntity"/>
      /// </summary>
      ...
      private EntityReference systemUserRef;
      ...
      /// <param name="traceLogBuilder">Trace Log Builder</param>
      /// <param name="unsecureConfig">The unsecure config.</param>
      /// <param name="secureConfig">The secure config.</param>
      ...
      public PluginContext(
        ...
        string registeredEntityLogicalName,
        Func<IOrganizationService, TContext> orgContextBuilder,
        Func<IPluginContext<TContext>, ITracingService, TTraceLog> traceLogBuilder,
        string unsecureConfig,
        string secureConfig,
        bool customVerboseLogSwitchedOn)
      {
      ...
      public TraceLogBase TraceLog => this.traceLog.Value;
      ...
      /// <inheritdoc />
      /// <summary>
      /// Gets the stage code of plug-in execution.
      /// </summary>
      /// <value>
      /// The stage code.
      /// </value>
      ...
      /// </value>
      public bool DepthIs1 => this.Depth == 1;
      ...
      /// </value>
      public EntityReference InitiatingUserRef => new EntityReference("systemuser", this.InitiatingUserId);
      ...
      /// <summary>
      /// Gets the grand parent execution context.
      /// </summary>
      /// <value>
      /// The execute context.
      /// </value>
      public IPluginExecutionContext GrandParentExecContext => this.ExecContext.ParentContext?.ParentContext;
      ...
      /// </value>
      public bool IsPreValidation => this.Stage == (int)Constants.Stage.PreValidate;
      ...
      /// </value>
      public bool IsPreOperation => this.Stage == (int)Constants.Stage.PreOperation;
      ...
      /// </value>
      public bool IsPostOperation => this.Stage == (int)Constants.Stage.PostOperation;
      ...
      /// </value>
      public bool IsAsynchronous => this.Mode == (int)Constants.Mode.Asynchronous;
      ...
      public EntityExt<TEntity> TargetEntityExt => 
        this.targetExt ?? (this.targetExt = new EntityExt<TEntity>(this.TargetEntity, this.PreImageEntity, true));
      ...
      public EntityExt<TEntity> PostImageExt => 
        this.postImageExt ?? (this.postImageExt = new EntityExt<TEntity>(this.PostImageEntity, this.PreImageEntity, false));
      ...
      /// The pre image.
      /// </value>
      public TEntity PreImageEntity => 
        this.preImageEntity 
          ?? (this.preImageEntity = this.PreImageAsBaseEntity?.ToEntity<TEntity>());

      /// <summary>
      /// Gets the pre image as <see cref="Entity"/>.
      /// </summary>
      /// <value>
      /// The pre image.
      /// </value>
      public Entity PreImageAsBaseEntity =>
        this.preImageAsBaseEntity 
          ?? (this.preImageAsBaseEntity = this.GetPreImageAsBaseEntity());
      ...
      /// The post image.
      /// </value>
      public TEntity PostImageEntity => 
        this.postImageEntity 
          ?? (this.postImageEntity = this.PostImageAsBaseEntity?.ToEntity<TEntity>());

      /// <summary>
      /// Gets the post image as <see cref="Entity"/>.
      /// </summary>
      /// <value>
      /// The post image.
      /// </value>
      public Entity PostImageAsBaseEntity =>
        this.postImageAsBaseEntity 
          ?? (this.postImageAsBaseEntity = this.GetPostImageAsBaseEntity());
      ...
      public EntityReference Assignee => 
        this.assignee 
          ?? (this.assignee = this.GetInputParameter<EntityReference>(AssigneeKey));
      ...
      public bool IsCreateMode => 
        (bool)(this.isCreateMode ?? (this.isCreateMode = this.GetIsMessage(Constants.MessageName.Create)));

      public bool IsUpdateMode => 
        (bool)(this.isUpdateMode ?? (this.isUpdateMode = this.GetIsMessage(Constants.MessageName.Update)));

      public bool IsDeleteMode => 
        (bool)(this.isDeleteMode ?? (this.isDeleteMode = this.GetIsMessage(Constants.MessageName.Delete)));

      public EntityReference SystemUserRef => 
        this.systemUserRef ?? (this.systemUserRef = UserHelper.GetSystemUserRef(this));
      ...
      public void TraceValue(string name, object value)
      {
        this.Trace("  {0}={1}", name, value);
      }
      ...
      public void TraceValue(string name1, object value1, string name2, object value2)
      {
        this.Trace("  {0}={1}, {2}={3}", name1, value1, name2, value2);
      }
      ...
      public void TraceValue(string name1, object value1, string name2, object value2, string name3, object value3)
      {
        this.Trace("  {0}={1}, {2}={3} {4}={5}", name1, value1, name2, value2, name3, value3);
      }
      /// <inheritdoc />
      /// <summary>
      ...
      public void TraceWithElapsedTimeEnter(string name)
      {
        this.TraceWithElapsedTime("Enter {0}.", name);
      }

      public void TraceWithElapsedTimeExit(string name)
      {
        this.TraceWithElapsedTime("Exit {0}.", name);
      }

      public void WrapInTraceWithElapsedTime(Action action, string name)
      {
        this.TraceWithElapsedTime("Enter {0}.", name);
        action();
        this.TraceWithElapsedTime("Exit {0}.", name);
      ...
      /// Pre Image Entity
      /// </returns>
      public Entity GetPreImageAsBaseEntity(string name = null)
      {
      ...
      /// Pre Image
      /// </returns>
      public TEntity GetPreImageEntity(string name = null)
      {
        var entity = this.GetPreImageAsBaseEntity(name);
      ...
      /// Post Image Entity
      /// </returns>
      public Entity GetPostImageAsBaseEntity(string name = null)
      {
      ...
      /// Post Image
      /// </returns>
      public TEntity GetPostImageEntity(string name = null)
      {
        var entity = this.GetPostImageAsBaseEntity(name);
        return entity?.ToEntity<TEntity>();
      }
      ...
      private Entity GetImageEntity(EntityImageCollection images, string name)
      {
        ...
        return images.TryGetValue(name, out var result) ? result : null;
      }
    }
    ...
    public class PluginStep
    {
      ...
      /// <param name="stage">The stage.</param>
      /// <param name="mode">The mode.</param>
      /// <param name="pluginMessage">Name of the plugin message.</param>
      /// <param name="handler">The handler.</param>
      public PluginStep(Stage? stage, Mode? mode, string pluginMessage, Action<PluginContext> handler)
      {
        this.Stage = stage;
        this.Mode = mode;
      ...
      /// The handler.
      /// </value>
      private readonly Action<PluginContext> handler;

      /// <summary>
      /// Executes the handler.
      /// </summary>
      /// <param name="pluginCtx">The plugin CTX.</param>
      public void Execute(PluginContext pluginCtx)
      {
        this.handler(pluginCtx);
      }
    }
  }
}